#!/bin/bash
# DeMoA + CCLIP — the paper's headline aggregator. Decisive test.
set -e
cd "$(dirname "$0")/.."
source .venv/bin/activate

PROGRESS=results/v2/cclip_progress.txt
mkdir -p results/v2/baselines
echo "DeMoA + CCLIP batch started $(date)" > "$PROGRESS"
...
cfg = BaselineConfig(
    ...
    p=0.5, aggregator="cclip", cclip_tau=100.0, cclip_L=1,
...
run_one clean_CCLIP_DeMoA          ""
run_one dorm_CCLIP_DeMoA_c80_T20   "dormancy_T_dark=20, dormancy_client_indices=[$COHORT_80], dormancy_payload='stealth_lie',"
