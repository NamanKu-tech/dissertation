#!/bin/bash
# DeMoA + Median — the aggregator that resists point-mass identical Byzantines.
set -e
cd "$(dirname "$0")/.."
source .venv/bin/activate

PROGRESS=results/v2/demoa_median_progress.txt
mkdir -p results/v2/baselines
echo "DeMoA + Median batch started $(date)" > "$PROGRESS"
...
cfg = BaselineConfig(
    ...
    p=0.5, aggregator="median", aggregator_f=0,
...
run_one clean_Median_DeMoA          ""
run_one dorm_Median_DeMoA_c80_T20   "dormancy_T_dark=20, dormancy_client_indices=[$COHORT_80], dormancy_payload='stealth_lie',"
